...
(() => {
  ...
  function getReact() {
    ...
    if (!R) throw new Error("app: window.React is not available yet");
    return R;
  }
  function getReactDOM() {
    const RD = window.ReactDOM;
    if (!RD) throw new Error("app: window.ReactDOM is not available yet");
    return RD;
  }
  ...
  var BASE_CSS = `
    ...
    /* Mirrors PRINT_BASELINE_CSS in apps/web deck-stage-export.ts \u2014 keep both
       in sync until runtime regains a build step. */
    @media print {
      @page { margin: 0.5cm; }
      ...
      render() {
        ...
        if (this.state.__err) {
          return h(
            ...
            h(
              "div",
              { className: "sc-logic-error", "data-runtime-chrome": "" },
